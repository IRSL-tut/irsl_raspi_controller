...
## ライブラリ等のinstall
- 必要ライブラリのインストール
    - micro sdをraspiにさして，起動後ログインして以下コマンドを実行する
        ```
        sudo apt update
        sudo apt upgrade -y
        ```
    ...
    - 再度ログインして以下コマンドを実行する
        ```
        sudo apt update
        sudo apt install -y git ubuntu-desktop
        sudo gpasswd -a $USER dialout
        sudo gpasswd -a $USER video
        ```
    - 再起動する
    - GUIが立ち上がるので，設定したユーザおよびパスワードでログインする．
        - not listed?を押してユーザおよびパスワードを入れる
- ros-oのインストール
...
- 必要ソフトウェアインストール(apt)
    ```
    sudo apt install -y ssh python3-pip libyaml-cpp-dev vcstool ros-one-image-transport ros-one-angles ros-one-controller-manager ros-one-joint-limits-interface ros-one-transmission-interface  ros-one-ros-control ros-one-ros-controllers python-is-python3
    ```
